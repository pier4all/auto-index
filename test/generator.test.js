...
const jsonAggLogic = '{ "aggregate": "test3", "collection": "test_collection", "pipeline": [{"$sort": {"salary": 1}}, {"$match": {"$or": [{"age": 30}, {"age": 35}, {"vip": true}, {"$unknown_op": {}}]}}]}'
...
tap.test('generate indexes for match stage', async (childTest) => {
  ...
  const expectedIndex = {"name":"test_index","key":{"name":1,"active":1},"collection":"test_collection","options":{}} 
...
tap.test('generate indexes for match stage with logic operations', async (childTest) => {
  ...
  const expectedIndex = {"name":"test_index","key":{"age": 1, "vip": 1},"collection":"test_collection","options":{}} 
